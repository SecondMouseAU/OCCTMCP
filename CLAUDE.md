...
- **OCCTSwiftScripts** ≥ 1.4.1: provides `occtkit` (only used by `execute_script` and `export_scene`); also ships `ScriptHarness` + `DrawingComposer` consumed in-process. `ExecuteScriptTool.scriptsPin` must track this pin (#42) and points at the SecondMouseAU URL. **Deliberately held below the actual latest (v1.5.0):** v1.5.0 still caps its own OCCTSwiftIO dependency to `<1.1.0`, which conflicts with OCCTSwiftTools ≥1.6.1's own OCCTSwiftIO `>=1.7.0` requirement (below) and makes the two unresolvable together. Filed as SecondMouseAU/OCCTSwiftScripts#80; bump past 1.5.0 once that ships a compatible OCCTSwiftIO range. Until then this repo runs the pre-BRepGraph-rename ScriptHarness/DrawingComposer, one minor behind the rest of the cohort
