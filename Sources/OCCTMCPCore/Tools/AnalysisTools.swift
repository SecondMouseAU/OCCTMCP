...
public enum AnalysisTools {
    ...
    private static func buildFeatureReport(
        ...
        func distinct(_ occurrenceIndex: Int) -> Int {
            guard occurrenceIndex >= 0, occurrenceIndex < aag.nodes.count else {
        ...
        let holes =
            ...
            ? aag.detectHoles().map {
                FeatureReport.Hole(
                    faceIndex: distinct($0.faceIndex), radius: $0.radius, depth: $0.depth)
            } : []
    ...
    public static func analyzeClearance(
        ...
        for i in 0..<loaded.count {
            for j in (i + 1)..<loaded.count {
                ...
                if computeContacts {
                    guard
                        let solutions = a.shape.allDistanceSolutions(to: b.shape, maxSolutions: 16)
                    else {
                        continue
                    }
    ...
    public struct GraphSelectFaceAdj: Encodable {
        public let face1: Int
        public let face2: Int
        public let convexity: String
        public let sharedEdgeCount: Int
    }
        ...
        do {
            ...
            case "edge-faces":
                ...
                guard let m = edge, m >= 0, m < graph.edgeCount else {
                    return .init(
                        "edge-faces requires `edge` in 0..<\(graph.edgeCount)", isError: true)
                }
                return IntrospectionTools.encode(
                    GraphSelectEdgeFaces(
            ...
            case "vertex-edges":
                ...
                guard let k = vertex, k >= 0, k < graph.vertexCount else {
                    return .init(
                        "vertex-edges requires `vertex` in 0..<\(graph.vertexCount)", isError: true)
                }
                return IntrospectionTools.encode(
            ...
            case "edges-class":
                ...
                guard ["boundary", "non-manifold", "seam", "degenerate"].contains(kind) else {
                    return .init(
                        "`class` must be boundary | non-manifold | seam | degenerate", isError: true
                    )
                }
                let matches: [Int] = (0..<graph.edgeCount).filter { i in
                    ...
                    case "seam":
                        return graph.edgeCoEdges(i).contains { graph.coedgeSeamPair($0) != nil }
                    case "degenerate": return graph.isEdgeDegenerated(i)
                    default: return false
                    }
                }
                return IntrospectionTools.encode(
                    GraphSelectEdgesClass(edgeClass: kind, edges: matches))

            default:
                return .init(
                    "Unknown query '\(query)'. Use face-neighbors | edge-faces | vertex-edges | face-adjacency | edges-class.",
                    isError: true)
            }
        } catch {
