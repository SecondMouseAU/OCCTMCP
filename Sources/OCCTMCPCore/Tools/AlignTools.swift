...
public enum AlignTools {
    ...
    public struct AlignReport: Encodable {
        ...
        /// 4 rows x 4 columns, ROW-MAJOR: `transform[i]` is row `i`.
        ///
        /// Maps a SOURCE-body point
    ...
    public static func alignBodies(
        ...
        let loadedSource:
            (manifest: ScriptManifest, body: BodyDescriptor, shape: Shape, path: String)
        ...
        if let cap = correspondenceDistanceCapMm, cap <= 0 {
            return .init(
                "correspondenceDistanceCapMm must be positive when supplied.", isError: true)
        }
        ...
        guard let sourceMesh = sourceShape.mesh(parameters: meshParams),
            sourceMesh.triangleCount > 0
        else {
            return .init("Failed to tessellate '\(bodyId)' for alignment.", isError: true)
        }
        guard let referenceMesh = referenceShape.mesh(parameters: meshParams),
            referenceMesh.triangleCount > 0
        else {
        ...
        if apply {
            ...
            guard let transformedShape = sourceShape.transformed(matrix: matrix12) else {
                return .init(
                    "apply=true requested, but applying the recovered transform to '\(bodyId)' failed.",
                    isError: true)
            }
            ...
            do {
                try Exporter.writeBREP(
                    shape: transformedShape, to: URL(fileURLWithPath: outputPath))
            } catch {
                return .init(
    ...
    /// `gTransformed(matrix:)` (gp_GTrsf, general/non-rigid) uses.
    ///
    /// Confirmed against both bridge
    ...
    static func axisAngle(fromRotationRows r: [[Double]]) -> (
        axis: SIMD3<Double>, angleDegrees: Double
    ) {
