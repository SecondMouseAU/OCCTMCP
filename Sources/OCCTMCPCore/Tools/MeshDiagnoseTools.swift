...
public enum MeshDiagnoseTools {
    ...
    public static func meshDiagnose(
        ...
        guard weldToleranceMm >= 0 else {
            return .init("weldToleranceMm must be >= 0.", isError: true)
        }
        ...
        let shown = Array(report.components.prefix(componentDisplayCap))
            .map {
                DiagnoseReport.ComponentEntry(triangleCount: $0.triangleCount, areaMm2: $0.area)
            }
        ...
        let diag = DiagnoseReport(
            ...
            components: shown,
            minAngleDegrees: .init(
                min: report.minAngleDegrees.min, p05: report.minAngleDegrees.p05),
    ...
    static func buildChecks(_ report: MeshIntegrityReport) -> [DiagnoseReport.CheckEntry] {
        ...
        if report.nonManifoldEdgeCount > 0 {
            checks.append(
                .init(
                    ...
                    detail:
                        "Not evaluated: orientability is only meaningful over 2-triangle edges, and this mesh has \(report.nonManifoldEdgeCount) non-manifold edge(s)."
                ))
        } else {
            checks.append(
                .init(
        ...
        let sliverBad =
            report.minAngleDegrees.p05 < sliverMinAngleFloorDegrees
        ...
        checks.append(
            .init(
                ...
                detail:
                    "minAngleDegrees.p05=\(fmt(report.minAngleDegrees.p05))\u{b0}, aspectRatio.p95=\(fmt(report.aspectRatio.p95))"
