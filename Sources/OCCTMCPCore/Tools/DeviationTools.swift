...
public enum DeviationTools {
    ...
    /// surface each sample CORRESPONDS to.
    ///
    /// Against a watertight reference those
    ...
    public struct DirectionStat: Encodable {
        ...
        /// trustworthy.
        ///
        /// The unsigned figures use all `samples`; a magnitude stays
        ...
        /// disagreeing).
        ///
        /// Excluded from signedMean/Min/Max.
        public let ambiguousSamples: Int
        /// `ambiguousSamples / samples`.
        ///
        /// Near 1.0 means the reference is open /
    ...
    /// samples.
    ///
    /// A near-constant non-zero `signedMean` across stations is the
    ...
    public struct DeviationReport: Encodable {
        ...
        /// Deviation of the surface of `from` measured against `to` (over-extension).
        public let fromToTo: DirectionStat
        /// Deviation of the surface of `to` measured against `from` (under-coverage).
        ...
        /// Forward (from→to) signed samples binned along `sectionAxis`.
        ///
        /// Present
    ...
    public static func measureDeviation(
        ...
        guard
            let (fwd, fwdSamples) = directedStats(
                source: fromTris, target: toTris, maxSamples: maxSamples, signMode: signMode)
        else {
            return .init("Deviation computation failed (empty tessellation).", isError: true)
        }
        guard
            let (rev, _) = directedStats(
                source: toTris, target: fromTris, maxSamples: maxSamples, signMode: signMode)
        else {
            return .init("Deviation computation failed (empty tessellation).", isError: true)
        }
        ...
        if let axis = sectionAxis, sectionCount >= 2, simd_length(axis) > 1e-12 {
            sections = sectionize(
                samples: fwdSamples, axis: simd_normalize(axis), bins: sectionCount)
        }
    ...
    struct TriMesh {
        ...
        /// normals.
        ///
        /// This is the sample's own surface orientation, which `.robust`
    ...
    /// Directed deviation with sign.
    ///
    /// Returns the aggregate `DirectionStat` plus
    /// the per-sample signed list (for sectioning / histograms).
    static func directedStats(
        source: TriMesh, target: TriMesh, maxSamples: Int,
        signMode: SignMode = .robust
    )
        -> (DirectionStat, [SignedSample])?
    {
        ...
        let stat = DirectionStat(
            ...
            signedMean: signedVals.isEmpty
                ? nil : signedVals.reduce(0, +) / Double(signedVals.count),
    ...
    private struct Candidate {
        ...
        let positive: Bool
        /// Does this triangle's outward normal agree with the sample's own?
        ///
        /// False
    ...
    private static func candidate(
        _ hit: TriBVH.NearestHit, target: TriMesh, p: SIMD3<Double>, srcN: SIMD3<Double>?
    ) -> Candidate {
    ...
    /// Signed distance from `p` to the target surface, via the EXACT
    /// k-nearest-TRIANGLE search on `target.bvh` (#116), not an approximation
    /// through nearby vertices, which can miss a large/sparse triangle whose
    /// closest point sits far from all three of its own vertices.
    ///
    ...
    static func signedQuery(
        ...
        return SignedHit(
            nearest: closest.distance, signed: signedDist,
    ...
    /// a reference `TriMesh`.
    ///
    /// Used by the heatmap (per-triangle centroid) and the
    ...
    static func signedDistances(
        ...
        assert(
            normals == nil || normals!.count == points.count,
            "signedDistances: normals must be 1:1 with points")
        var out = [SignedHit](
            repeating: SignedHit(nearest: 0, signed: 0, ambiguous: false),
            count: points.count)
    ...
    /// numerically comparable against each other (#125).
    ///
    /// This is the
