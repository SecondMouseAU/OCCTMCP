...
public enum AnnotationsRenderer {
    ...
    @available(
        *, deprecated,
        message:
            "v0.10 removed the sphere-per-point fallback; PointConverter handles arbitrary cloud sizes."
    )
    public static let maxPointCloudPoints = Int.max

    /// Synthesise ViewportBodies for every renderable primitive + dimension in the sidecar.
    ///
    /// Bodies are tagged with the primitive/dimension id and a
    ...
    /// v0.9: translate sidecar dimensions into ViewportMeasurement values that
    /// OffscreenRenderer's headless overlay pass renders as proper 2D leader
    /// lines, arrow tips, and value labels.
    ///
    /// Mapping is direct:
        ...
        for (i, (dir, color)) in axes.enumerated() {
            guard
                let cyl = Shape.cylinder(
                    at: origin, direction: dir, radius: armRadius, height: length)
            else {
                continue
            }
    ...
    private static func boundingBox(_ prim: PrimitiveAnnotation) -> ViewportBody? {
        ...
        for (origin, dir, length) in corners {
            guard length > 1e-6,
                let cyl = Shape.cylinder(
                    at: origin, direction: dir, radius: edgeRadius, height: length)
            else {
                continue
            }
            edges.append(cyl)
    ...
    private static func diffMarker(_ prim: PrimitiveAnnotation) -> ViewportBody? {
        guard let center = vec3(prim.params["center"]),
            let extent = vec3(prim.params["extent"])
        else { return nil }
    ...
    private static func pointCloud(_ prim: PrimitiveAnnotation) -> ViewportBody? {
        ...
        if case .array(let colorEntries)? = prim.params["colors"],
            colorEntries.count == positions.count
        {
    ...
    private static func dimension(_ dim: DimensionAnnotation) -> ViewportBody? {
        ...
        case "angular":
            ...
            return angularDimension(
                armA: world[0], apex: world[1], armB: world[2], id: dim.id, color: color)
        case "radial":
    ...
    private static func linearDimension(
        ...
        guard let leader = Shape.cylinder(at: from, direction: dir, radius: radius, height: length)
        else { return nil }
    ...
    private static func angularDimension(
        ...
        var pieces: [Shape] = []
        if let c = Shape.cylinder(
            at: apex, direction: simd_normalize(dirA), radius: armARadius, height: lenA)
        {
            pieces.append(c)
        }
        if let c = Shape.cylinder(
            at: apex, direction: simd_normalize(dirB), radius: armBRadius, height: lenB)
        {
            pieces.append(c)
        }
    ...
    private static func radialMarker(at center: SIMD3<Double>, id: String, color: SIMD4<Float>)
        -> ViewportBody?
    {
    ...
    private static func makeViewportBody(_ shape: Shape, id: String, color: SIMD4<Float>)
        -> ViewportBody?
    {
        ...
        let (vb, _) = CADFileLoader.shapeToBodyAndMetadata(
            shape, id: id, color: color, directMesh: true)
        return vb
    }
}
