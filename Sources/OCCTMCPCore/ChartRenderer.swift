...
enum ChartRenderer {
    ...
    /// colormap above (which encodes a signed SCALAR) doesn't apply.
    ///
    /// The first 8 are the Okabe-Ito set (with pure black swapped for a mid grey,
    ...
    /// crash.
    ///
    /// Callers are responsible for warning when `index >=
    ...
    static func histogram(
        ...
        guard let ctx = makeContext(width: width, height: height, background: SIMD4(1, 1, 1, 1))
        else {
        ...
        drawText(
            title, at: CGPoint(x: 12, y: CGFloat(height) - 26), fontSize: 15, color: ink, in: ctx)

        guard !values.isEmpty else {
            drawText(
                "no samples", at: CGPoint(x: margin.midX, y: margin.midY), fontSize: 13, color: ink,
                in: ctx, anchor: .center)
        ...
        func px(_ v: Double) -> CGFloat {
            margin.minX + CGFloat((v - lo) / (hi - lo)) * margin.width
        }
        func py(_ c: Int) -> CGFloat {
            margin.minY + CGFloat(Double(c) / Double(maxCount)) * margin.height
        }

        // ±tolerance band
        if let tol = tolerance, tol > 0 {
            let x0 = px(max(lo, -tol))
            let x1 = px(min(hi, tol))
            ctx.setFillColor(cg(band))
            ctx.fill(CGRect(x: x0, y: margin.minY, width: max(0, x1 - x0), height: margin.height))
        }

        // axes
        ctx.setStrokeColor(cg(grid))
        ctx.setLineWidth(1)
        ctx.stroke(margin)

        // zero line
        if lo < 0 && hi > 0 {
            ctx.setStrokeColor(cg(SIMD4(0.5, 0.5, 0.5, 1)))
        ...
        let fmt = "%.3g"
        drawText(
            String(format: fmt, lo), at: CGPoint(x: margin.minX, y: margin.minY - 16), fontSize: 11,
            color: ink, in: ctx, anchor: .center)
        drawText(
            String(format: fmt, hi), at: CGPoint(x: margin.maxX, y: margin.minY - 16), fontSize: 11,
            color: ink, in: ctx, anchor: .center)
        if lo < 0 && hi > 0 {
            drawText(
                "0", at: CGPoint(x: px(0), y: margin.minY - 16), fontSize: 11, color: ink, in: ctx,
                anchor: .center)
        }
        drawText(
            "count (peak \(maxCount))", at: CGPoint(x: margin.minX, y: margin.maxY + 6),
            fontSize: 11, color: ink, in: ctx)
    ...
    struct ProfileLayer {
        ...
        init(
            loops: [[SIMD2<Double>]], openPaths: [[SIMD2<Double>]] = [],
    ...
    static func profileOverlay(
        ...
        let ink = SIMD4<Float>(0.12, 0.12, 0.14, 1)
        drawText(
            title, at: CGPoint(x: 12, y: CGFloat(height) - 24), fontSize: 14, color: ink, in: ctx)
        ...
        guard any else {
            drawText(
                "no contours at this station",
                at: CGPoint(x: CGFloat(width) / 2, y: CGFloat(height) / 2), fontSize: 13,
                color: ink, in: ctx, anchor: .center)
            try finalize(ctx, to: url)
            return
        }
        ...
        let pad: CGFloat = 48
        let plot = CGRect(
            x: pad, y: pad, width: CGFloat(width) - 2 * pad, height: CGFloat(height) - 2 * pad)
        ...
        for layer in layers {
            ...
            ctx.setFillColor(cg(layer.color))
            ctx.fill(sw)
            drawText(
                layer.label, at: CGPoint(x: CGFloat(width) - 128, y: ly), fontSize: 12, color: ink,
                in: ctx)
            ly -= 18
        }
        // scale note
        drawText(
            String(format: "%.3g × %.3g mm", size.x, size.y),
    ...
    static func overlayColorbar(on pngURL: URL, minValue: Double, maxValue: Double, label: String)
        throws
    {
        ...
        ctx.setStrokeColor(cg(ink))
        ctx.setLineWidth(1)
        ctx.stroke(CGRect(x: barX, y: barY, width: barW, height: barH))

        let lx = barX + barW + 6
        drawText(
            String(format: "%+.3g", maxValue), at: CGPoint(x: lx, y: barY + barH - 6), fontSize: 11,
            color: ink, in: ctx)
        drawText(
            String(format: "%+.3g", minValue), at: CGPoint(x: lx, y: barY), fontSize: 11,
            color: ink, in: ctx)
        ...
        // background instead.
        drawText(
            label, at: CGPoint(x: CGFloat(w) - 8, y: barY + barH + 6),
    ...
    /// entry) and rewrite the file in place.
    ///
    /// Mirrors `overlayColorbar`'s load/draw/rewrite shape for a categorical
    /// (rather than scalar) key. Rows are capped to what the image height
    /// allows; anything past that collapses into a final "+N more" line
    /// rather than running off the bottom or silently dropping entries with
    /// no trace.
    static func overlayZoneLegend(on pngURL: URL, entries: [(label: String, color: SIMD4<Float>)])
        throws
    {
        ...
        if entries.count > shown {
            ...
            drawText(
                "+\(entries.count - shown) more", at: CGPoint(x: labelX, y: y - 9), fontSize: 10,
                color: ink, in: ctx)
        }
    ...
    static func stripChart(
        ...
        guard !stations.isEmpty else {
            drawText(
                "no stations", at: CGPoint(x: margin.midX, y: margin.midY), fontSize: 13,
                color: ink, in: ctx, anchor: .center)
            try finalize(ctx, to: url)
            return
        }
        ...
        var ylo = min(0, ys.min()!)
        var yhi = ys.max()!
        if yhi - ylo < 1e-9 { yhi += 0.5 }

        func px(_ v: Double) -> CGFloat {
            margin.minX + CGFloat((v - xlo) / (xhi - xlo)) * margin.width
        }
        func py(_ v: Double) -> CGFloat {
            margin.minY + CGFloat((v - ylo) / (yhi - ylo)) * margin.height
        }
        ...
        if let tol = tolerance, tol >= ylo, tol <= yhi {
            ctx.setFillColor(cg(band))
            let y0 = py(max(ylo, -tol))
            let y1 = py(min(yhi, tol))
            ctx.fill(
                CGRect(x: margin.minX, y: min(y0, y1), width: margin.width, height: abs(y1 - y0)))
            ctx.setStrokeColor(cg(SIMD4(0.35, 0.6, 0.3, 1)))
            ctx.setLineWidth(1)
            ctx.beginPath()
        ...
        for s in sorted {
            ctx.fillEllipse(
                in: CGRect(x: px(s.axisCoord) - 2, y: py(s.value) - 2, width: 4, height: 4))
        }

        let fmt = "%.3g"
        drawText(
            String(format: fmt, xlo), at: CGPoint(x: margin.minX, y: margin.minY - 16),
            fontSize: 11, color: ink, in: ctx, anchor: .center)
        drawText(
            String(format: fmt, xhi), at: CGPoint(x: margin.maxX, y: margin.minY - 16),
            fontSize: 11, color: ink, in: ctx, anchor: .center)
        drawText(
            String(format: fmt, ylo), at: CGPoint(x: margin.minX - 8, y: margin.minY), fontSize: 11,
            color: ink, in: ctx, anchor: .right)
        drawText(
            String(format: fmt, yhi), at: CGPoint(x: margin.minX - 8, y: margin.maxY - 10),
            fontSize: 11, color: ink, in: ctx, anchor: .right)
        drawText(
            yLabel, at: CGPoint(x: margin.minX, y: margin.maxY + 6), fontSize: 11, color: ink,
            in: ctx)
        drawText(
            "axisCoord (mm)", at: CGPoint(x: margin.maxX, y: margin.minY - 32), fontSize: 11,
            color: ink, in: ctx, anchor: .right)
