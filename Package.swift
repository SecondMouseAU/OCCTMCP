...
let package = Package(
    ...
    dependencies: [
        ...
        occtDep("OCCTSwiftScripts", from: "1.6.0"),  // >=1.6.0 (#171): repin OCCTSwift floor to 2.0.0; fixes the confirmed selfIntersectionCount break (OCCTSwift#763, Heal/GraphValidate now use opt-in isSelfIntersecting(timeout:) via a new --self-intersection-timeout flag) and a second real AAG occurrence-index bug (OCCTSwift#642) in FeatureRecognize/GraphSelect/GraphML. Known issue, not blocking: recipe 02's spring volume reads ~14% high under 2.0.0, looks like a genuine OCCT kernel regression in pipeShell on a helix spine, filed as OCCTSwift#830, unrelated to anything execute_script/export_scene calls here.
